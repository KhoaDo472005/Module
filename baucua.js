...
module.exports.run = async function ({ api, event, args, Currencies }) {
  ...
  var ketqua = [];
  for (let n= 0; n<3; n++) {
    ketqua.push(allface[Math.floor(Math.random() * allface.length)]);
  };
  if (luckynumber <= tlt && !ketqua.includes(datcuoc)) ketqua[Math.floor(Math.random() * 0x3)] = datcuoc; 
