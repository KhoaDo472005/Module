...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (type == "encode" || type == "en") return api.sendMessage(`Mã hóa hoàn tất: ${encode(input)}`, threadID, messageID);
  if (type == "decode" || type == "de") return api.sendMessage(`Giải mã hoàn tất: ${decode(input)}`, threadID, messageID);
}
