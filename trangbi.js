...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (args[0] == "bị") return api.sendMessage("Vui lòng ghi đúng tên lệnh!", threadID, messageID);
  var search = event.body.slice(event.body.indexOf(" ") + 1).toLowerCase();
  if (args.length < 2 || args.length > 4) return api.sendMessage(`Không có dữ liệu cho trang bị ${search}!`, threadID, messageID);
